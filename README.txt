cloudmig is an open source tool to help migrate considerable amounts of data
from a cloud-storage account to another, using the Scality Droplet library.

The whole project is released under a 3-clause Modified BSD Licence.

All source code is hosted here, and the documentation will be available at
http://github.com/Joacchim/Scality-Cloud-Migration-Tool/wiki

Please report bugs, comments and questions on the project's issues page :
http://github.com/Joacchim/Scality-Cloud-Migration-Tool/issues

The Scality Droplet Library's code is hosted at
http://github.com/scality/Droplet
